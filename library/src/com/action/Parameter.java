...
public class Parameter extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -181745671080433363L;
	...
	private void parameterModify(HttpServletRequest request,HttpServletResponse response) 
	throws ServletException, IOException {
		...
		if (ret == 0) {
			...
			request.getRequestDispatcher("parameter_ok.jsp?para=2").forward(request,response);
		}
	...
	throws ServletException, IOException {
		...
		request.getRequestDispatcher("parameter_modify.jsp").forward(request,response);
	}
